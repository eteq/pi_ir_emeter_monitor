...
fn main() -> Result<(), Box<dyn Error>> {
    let log_path = "power_mon.log";
    let mut outfile = OpenOptions::new().append(true).create(true).open(log_path)?;
    ...
    loop {
        ...
        if 23 < since_last_edge_ms && since_last_edge_ms < 26 {
            ...
            //write!(outfile, "{}", unix_time_nanos)?;
            outfile.write_all(&unix_time_nanos.to_le_bytes())?;
            outfile.flush()?;
